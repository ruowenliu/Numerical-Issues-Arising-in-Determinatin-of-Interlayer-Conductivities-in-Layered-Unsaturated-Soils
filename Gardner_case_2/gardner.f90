...
integer :: numtime=0, num=0
integer :: multinum=0,pos_up_num=0,pos_low_num=0,dif_flux_num=0,fxpt_diver_num=0
integer :: p1, p2
...
logical :: upperx_sat, lowerx_sat, multi_root, incons_fl, fxpt_diver
...
cdint=(24d0/2d0+1d0)/25d0
p1 = floor(cdint/dz)
p2 = ceiling(cdint/dz)
!pl = floor((0.05d0-0.5d0*dz)/dz)
pl = nint((0.02d0-0.5d0*dz)/dz)
allocate(h(neq))
...
open(unit=30,file=prefix(1:ix)//'hbelow_neq24.bin',form='unformatted',access='stream')
open(unit=31,file=prefix(1:ix)//'wbelow_neq24.bin',form='unformatted',access='stream')
open(unit=32,file=prefix(1:ix)//'multitime.bin',form='unformatted',access='stream')
open(unit=33,file=prefix(1:ix)//'fxptdivtime.bin',form='unformatted',access='stream')
IERROR = 0
ITASK = 5
numtime = size(tspan)
...
!    end if
    ...
    if (fxpt_diver) then
      write(33) t0
      fxpt_diver_num = fxpt_diver_num+1
    end if
    num = num+1
  end do
...
!write(*,*) '# of positive lower h = ', pos_low_num
!write(*,*) '# of inconsistent flux = ', dif_flux_num
!write(*,*) '# of fixed-point 2-cycle = ', fxpt_diver_num
close(11)
close(21)
close(22)
close(23)
