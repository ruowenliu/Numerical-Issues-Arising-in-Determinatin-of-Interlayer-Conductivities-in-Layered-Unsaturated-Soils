...
real(8), dimension(2) :: a, b, n, thr, ths ! parameters
...
real(8) :: t0, tf, dt, rtol, atol, tol, tspan(8), hmx, tprev
...
!tht=0.36687d0
!thb=0.00102d0
!call H_THETA(1,1,tht,ht)
!call H_THETA(2,1,thb,hb)
!print*, ht
!print*, hb
!allocate(h(neq))
!h(neq/2+1:neq) = hb(1)
...
!  end if
  ...
  do while (t0<tf)
    ...
    num = num+1
!    if (tprev<5d-3*dble(initfile+1) .and. t0>=5d-3*dble(initfile+1))  then
!    print*, t0
!initfile = initfile+1
!fileout(1:ix+2)=prefix(1:ix)//'w.'
!write(fileout(ix+3:ix+6),'(i4.4)') initfile
...
do k=0,neq
  if (kjm(k)/=0.0d0) then
     qjm(k)=kjm(k)*(1.0d0-(hj(k+1)-hj(k))/dz)
  else
     qjm(k)=0.0d0
  end if
end do
...
!$OMP PARALLEL DO
do k=1,neq/2
  if (theta(k)>ths(1)) then
    ydot(k) = dmin1(0.0d0,ydot(k))
  end if
  if (theta(neq/2+k)>ths(2)) then
    ydot(neq/2+k) = dmin1(0.0d0,ydot(neq/2+k))
  end if
end do
!$OMP END PARALLEL DO
return
...
real(8),intent(out) :: s(dim)
real(8) :: m
m = 1.0d0-1.0d0/n(layer)
s = (1.0d0+(dmax1(-a(layer)*h,0.0d0))**n(layer))**(-m)
return
end subroutine S_H
...
real(8) :: m
m = 1.0d0-1.0d0/n(layer)
h = (((dmin1(dmax1(s,0.0d0),1.0d0))**(-1.0d0/m)-1.0d0)**(1.0d0/n(layer)))/(-a(layer))
return
end subroutine H_S
...
m = 1.0d0-1.0d0/n(layer)
K = b(layer)*&
  dsqrt(dmin1(dmax1(s,0.0d0),1.0d0))*&
  ((1.0d0-(1.0d0-(dmin1(dmax1(s,0.0d0),1.0d0))**(1.0d0/m))**m)**2.0d0)
return
end subroutine K_S
